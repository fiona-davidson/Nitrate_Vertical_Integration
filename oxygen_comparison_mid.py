...
for ix in np.arange(lx):
        for iy in np.arange(ly):
            ...
            ym = model[bnd[0],iy,ix]#iy,ix].copy()
            yo = obs[bnd[0],iy,ix]#iy,ix].copy()
            ...
            if (len(ym) > 1) & (sum(ym) > 0):
                ...
                bz_m[iy, ix] = b_m
                print("b_o", np.mean(b_o), np.max(b_o))
...
outfile_obs = "/home/fid000/WORK7/ANALYSIS/model_evaluation/DATA/Integration/Oxygen/mid_obs_2016month"+str(i)+".nc"
outfile_model = "/home/fid000/WORK7/ANALYSIS/model_evaluation/DATA/Integration/Oxygen/mid_model_2016month"+str(i)+".nc"
