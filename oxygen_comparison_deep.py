...
for ix in np.arange(lx):
        for iy in np.arange(ly):
            ...
            ym = model[cnd[0],iy,ix]#iy,ix].copy()
            yo = obs[cnd[0],iy,ix]#iy,ix].copy()
            ...
            if (len(ym) > 1) & (sum(ym) > 0):
                ...
                cz_m[iy, ix] = c_m
                print("c_o", np.mean(c_o), np.max(c_o))
...
outfile_obs = "/home/fid000/WORK7/ANALYSIS/model_evaluation/DATA/Integration/Oxygen/deep_obs_2016month"+str(i)+".nc"
outfile_model = "/home/fid000/WORK7/ANALYSIS/model_evaluation/DATA/Integration/Oxygen/deep_model_2016month"+str(i)+".nc"
